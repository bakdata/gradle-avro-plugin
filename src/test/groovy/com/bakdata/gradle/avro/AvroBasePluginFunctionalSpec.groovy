...
import com.bakdata.gradle.avro.test.custom.CommentGenerator
import com.bakdata.gradle.avro.test.custom.TimestampGenerator
...
class AvroBasePluginFunctionalSpec extends FunctionalSpec {
    ...
    def "can generate java files from json schema"() {
        given:
        buildFile << """
        |tasks.register("generateAvroJava", com.bakdata.gradle.avro.GenerateAvroJavaTask) {
    ...
    def "can generate json schema files from json protocol"() {
        given:
        buildFile << """
        |tasks.register("generateSchema", com.bakdata.gradle.avro.GenerateAvroSchemaTask) {
        |    source file("src/main/avro")
        |    include("**/*.avpr")
    ...
    def "can generate json schema files from IDL"() {
        given:
        buildFile << """
        |tasks.register("generateProtocol", com.bakdata.gradle.avro.GenerateAvroProtocolTask) {
        |    source file("src/main/avro")
    ...
    def "supports classpath property for instantiating of velocity tools"() {
        ...
        |    additionalVelocityToolClasses = ['com.bakdata.gradle.avro.test.custom.TimestampGenerator',
        |                                     'com.bakdata.gradle.avro.test.custom.CommentGenerator']
        |}
        ...
        |}
        |tasks.register("generateAvro", com.bakdata.gradle.avro.GenerateAvroJavaTask) {
    ...
    private void copyAvroTools(String destDir) {
        copyFile("src/test/java", destDir,
            "com/bakdata/gradle/avro/test/custom/CommentGenerator.java")
        copyFile("src/test/java", destDir,
            "com/bakdata/gradle/avro/test/custom/TimestampGenerator.java")
    }
}
