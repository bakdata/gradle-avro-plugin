...
class CustomConversionFunctionalSpec extends FunctionalSpec {
    private void copyCustomConversion(String destDir) {
        copyFile("src/test/java", destDir,
            "com/bakdata/gradle/avro/test/custom/TimeZoneConversion.java")
        copyFile("src/test/java", destDir,
            "com/bakdata/gradle/avro/test/custom/TimeZoneLogicalType.java")
        copyFile("src/test/java", destDir,
            "com/bakdata/gradle/avro/test/custom/TimeZoneLogicalTypeFactory.java")
    }
    ...
    def "can use a custom conversion when generating java from a schema with stringType = \"String\""() {
        ...
        |import com.bakdata.gradle.avro.test.custom.*
        |avro {
        |    stringType = "String"
    ...
    def "can use a custom conversion from outside of the build classpath when generating java from a protocol"() {
        ...
        |    logicalTypeFactory("timezone", "com.bakdata.gradle.avro.test.custom.TimeZoneLogicalTypeFactory")
        |    customConversion("com.bakdata.gradle.avro.test.custom.TimeZoneConversion")
        |}
        |""".stripMargin()
        addDefaultRepository()
        addAvroDependency()
